...
export default async function handler(req, res) {
  ...
  // Schüler mit genau 1 Stunde Guthaben (keine Lehrer/Admins, kein Opt-out)
  const { data: low } = await sb.from('profiles')
    .select('id,name,email,credits,email_optout,is_admin,is_teacher')
    .eq('credits', 1);
  ...
  // Wer hat in den letzten 30 Tagen schon eine credit_offer-Mail bekommen?
  const since = new Date(Date.now() - 30*24*60*60*1000).toISOString();
  const { data: recent } = await sb.from('email_log')
    .select('user_id,sent_at').eq('kind','credit_offer').gte('sent_at', since)
    .in('user_id', targets.map(t => t.id));
  const blocked = new Set((recent || []).map(r => r.user_id));

  let sent = 0, errors = 0;
  for (const p of targets) {
    if (blocked.has(p.id)) continue;
    const { error: logErr } = await sb.from('email_log').insert({ kind:'credit_offer', ref:p.id, user_id:p.id });
    if (logErr) continue; // race / schon protokolliert

    const vorname = (p.name || '').split(' ')[0] || 'du';
    const html = brandedOfferEmail({ vorname, site, paketeUrl });
    ...
    const r = await fetch('https://api.brevo.com/v3/smtp/email', {
      ...
      body: JSON.stringify({
        ...
        subject: 'Nur noch 1 Stunde übrig – wie geht’s weiter? 💛',
        htmlContent: html,
      }),
    });
    if (r.ok) sent++; else { errors++; await sb.from('email_log').delete().eq('kind','credit_offer').eq('ref', p.id); }
  }
        ...
        <tr><td style="padding:8px 30px 4px">
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#FFF8E0;border:1px solid #F0E5D8;border-left:5px solid #2DD4BF;border-radius:16px">
            <tr><td style="padding:16px 20px;font-family:${ff};font-size:15px;line-height:1.6;color:#1A1A1A">
              ...
              Von der einzelnen Stunde bis zum monatlichen All-Inclusive-Pass – mit jedem Paket besuchst du <b>alle Clubs</b> (Sprechen, Grammatik, Wortschatz, Aussprache &amp; Sprachspiele).
            </td></tr>
          </table>
        </td></tr>
