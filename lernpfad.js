...
(function () {
  ...
  .lernpfad .lp-heute .lp-tx{flex:1;min-width:220px}
  .lernpfad .lp-heute small{font-family:var(--schrift-marker,cursive);color:var(--rot);font-size:15px;display:block;margin-bottom:2px}
  .lernpfad .lp-heute h2{font-size:24px;margin:0 0 4px}
  .lernpfad .lp-heute h2::after{display:none}
  .lernpfad .lp-heute p{color:var(--text-soft);font-size:15px;margin:0}
  .lernpfad .lp-heute .btn{flex:0 0 auto}

  .lernpfad .stat{background:var(--karte);border:2.5px solid var(--tinte);border-radius:16px;
    box-shadow:var(--schatten,3px 4px 0 rgba(32,33,31,.10));padding:14.5px;text-align:center}
  .lernpfad .stat b{display:block;font-size:24px;line-height:1.15;font-variant-numeric:tabular-nums}
  .lernpfad .stat span{font-size:12.5px;color:var(--text-soft)}
  .lernpfad .btn-primary{background:var(--tuerkis);color:var(--auf-tuerkis);
    border:2.5px solid var(--auf-tuerkis);box-shadow:2px 3px 0 rgba(6,49,56,.28)}
  .lernpfad .kpill{border:2.5px solid var(--tinte);background:var(--karte);color:var(--tinte);
    border-radius:var(--hand);padding:7.5px 14.5px;box-shadow:2px 3px 0 rgba(32,33,31,.12)}
  .lernpfad .kpill.active{background:var(--tuerkis);color:var(--auf-tuerkis);border-color:var(--auf-tuerkis)}
  .lernpfad .lp-reihe{display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:12px;margin-bottom:22px}

  /* Die Straße */
  .lernpfad .lp-kap{margin-bottom:26px}
  .lernpfad .lp-kap-kopf{display:flex;align-items:center;gap:12px;margin-bottom:10px}
  .lernpfad .lp-kap-kopf .lp-ic{font-size:24px;line-height:0;flex:0 0 auto}
  .lernpfad .lp-kap-kopf h3{margin:0;font-size:19px}
  .lernpfad .lp-kap-kopf i{font-style:normal;font-family:var(--schrift-notiz,cursive);font-size:17px;color:var(--text-soft)}
  .lernpfad .lp-kap-kopf .lp-zahl{margin-left:auto;font-family:var(--schrift-kopf,inherit);font-weight:700;
    font-size:13px;color:var(--tuerkis-dunkel);white-space:nowrap}

  .lernpfad .lp-strasse{position:relative;padding-left:30px}
  .lernpfad .lp-strasse::before{content:'';position:absolute;left:9px;top:12px;bottom:12px;width:4px;
    border-radius:3px;background:repeating-linear-gradient(180deg,var(--linie) 0 9px,transparent 9px 16px)}
  .lernpfad .lp-szene{position:relative;display:flex;align-items:center;gap:12px;
  ...
  .lernpfad .lp-szene:hover{transform:translateY(-2px)}
  .lernpfad .lp-szene::before{content:'';position:absolute;left:-30px;top:50%;transform:translateY(-50%);
    width:19px;height:19px;border-radius:50%;background:var(--karte);border:2.5px solid var(--tinte)}
  .lernpfad .lp-szene.fertig::before{background:var(--gruen)}
  .lernpfad .lp-szene.dran::before{background:var(--gelb)}
  .lernpfad .lp-szene .lp-em{font-size:20px;flex:0 0 auto}
  .lernpfad .lp-szene .lp-t{flex:1;min-width:0;font-family:var(--schrift-kopf,inherit);font-weight:700}
  .lernpfad .lp-szene .lp-d{font-family:var(--schrift-notiz,cursive);font-size:16px;color:var(--text-soft);flex:0 0 auto}
  .lernpfad .lp-szene.fertig{opacity:.72}
  .lernpfad .lp-szene.fertig .lp-t{text-decoration:line-through;text-decoration-thickness:2px}
  .lernpfad .lp-szene.dran{border-width:3px;box-shadow:4px 5px 0 rgba(32,33,31,.16)}
  .lernpfad .lp-haken{flex:0 0 auto;font-weight:700;color:var(--gruen-tief)}

  .lernpfad .lp-leer{background:var(--karte);border:2.5px dashed var(--linie);border-radius:14px;
    padding:13px 15px;margin-bottom:10px;font-family:var(--schrift-notiz,cursive);font-size:17px;color:var(--text-soft)}

  .lernpfad .lp-wahl{display:flex;gap:8px;flex-wrap:wrap;margin-bottom:20px}

  @media(max-width:620px){
    .lernpfad .lp-heute{padding:17px 16px;gap:14px}
    .lernpfad .lp-heute h2{font-size:20px}
    .lernpfad .lp-strasse{padding-left:26px}
    .lernpfad .lp-szene::before{left:-26px}
  }`;
  ...
  function pfad(zielId) {
    ...
    r.innerHTML = '<div class="lernpfad">' +
      ...
      heuteHtml(niveau) + standHtml(niveau) + pfadHtml(niveau) + '</div>';
    binde(r);
  }
