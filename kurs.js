...
(function () {
  ...
  /* Jedes Niveau hat seinen eigenen Zweig: a1, a2, b1 …
     A1 bleibt genau da, wo es immer lag — alter Fortschritt geht nicht verloren. */
  function zweig(id){ return String(id||KURSNIV||'A1').toLowerCase(); }
  function lekStand(nr,id){ var z=stand()[zweig(id)]||{}; return z[nr]||{}; }
  function lekMerken(nr,neu,id){
    var s=stand(), n=zweig(id), z=s[n]||{}, x=z[nr]||{}, k;
    for(k in neu) if(neu.hasOwnProperty(k)) x[k]=neu[k];
    z[nr]=x; s[n]=z; standSpeichern(s);
  }
  /* Eine Lektion hat sechs Teile. Üben zählt in Prozent, der Rest ganz oder gar nicht. */
  function lekProzent(nr,id){
    var x=lekStand(nr,id), t=[];
  ...
  function niveauProzent(id){
    var L=lektionen(id); if(!L.length) return 0;
    var sum=0,i; for(i=0;i<L.length;i++) sum+=lekProzent(L[i].nr,id);
  ...
  /* Das Niveau, in dem gerade gelesen wird. Wird gesetzt, sobald ein Kurs
     oder eine Lektion geoeffnet wird — alles darunter richtet sich danach. */
  var KURSNIV = 'A1';
  function nivName(id){ return String(id==null?KURSNIV:id).toUpperCase(); }
  function kursDaten(id){
    var d = window[nivName(id)];
    return (d && d.lektionen && d.lektionen.length) ? d : null;
  }
  function lektionen(id){ var d=kursDaten(id); return d?d.lektionen:[]; }
  function lektionVon(nr,id){ var a=lektionen(id),i; for(i=0;i<a.length;i++) if(a[i].nr===Number(nr)) return a[i]; return null; }
  function hatKurs(id){ return !!kursDaten(id); }
  /* Niveau setzen — nur, wenn es dafuer wirklich einen Kurs gibt */
  function nivSetzen(id){
    if(id && hatKurs(id)){ KURSNIV=nivName(id); S('niveau',KURSNIV); return true; }
    /* Ohne Angabe gilt das gemerkte Niveau — sonst landet man nach dem
       Neuladen wieder im A1-Kurs, obwohl man A2 macht. */
    var g=niveauGemerkt();
    KURSNIV = hatKurs(g) ? nivName(g) : (hatKurs(KURSNIV) ? nivName(KURSNIV) : 'A1');
    return false;
  }
  ...
  /* Je Niveau eine eigene Tabelle — „gesundheit" und „unterwegs" gibt es
     in A1 und in A2, sollen aber nicht dasselbe Foto bekommen. */
  var LEKBILD = {
    A1: {
    ...
    A2: {
      'ankommen':'th-migration',        'wohnung-suchen':'sz-wohnung',
      'im-haus':'th-nachbarn',          'arbeit-finden':'sz-bewerbung',
      'im-betrieb':'th-arbeit',         'beim-amt':'th-amt',
      'gesundheit':'th-arzt',           'geld-und-vertraege':'th-wirtschaft',
      'kinder-und-schule':'th-bildung', 'unterwegs':'sz-bahnhof',
      'freizeit-und-kontakte':'a1-freizeit', 'medien-und-technik':'th-digital',
      'essen-und-einladen':'sz-restaurant',  'plaene-und-zukunft':'th-gespraech'
    }
  };
  /* Faellt eine Datei aus, springt ein verwandtes Motiv ein statt eines
     kaputten Symbols. */
  var ERSATZBILD = {
    'a1-familie':'th-migration',  'a1-freizeit':'th-ironie',
    'a1-service':'th-gespraech',  'a1-kleidung':'th-einkauf',
    'a1-feste':'th-essen',        'sz-wohnung':'th-wohnung',
    'sz-bewerbung':'th-bewerbung','sz-bahnhof':'th-weg',
    'sz-restaurant':'th-essen',   'th-migration':'a1-l1',
    'th-nachbarn':'th-wohnung',   'th-wirtschaft':'th-arbeit',
    'th-digital':'th-medien',     'th-gespraech':'a1-l1'
  };
  function lekMotiv(l, id){
    var t = LEKBILD[nivName(id)] || LEKBILD.A1 || {};
    return (l && t[l.id]) || LEKBILD.A1[l && l.id] || 'a1-l1';
  }
  function lekBild(l, klein, id){
    return 'illu/' + lekMotiv(l,id) + (klein ? '-s' : '') + '.jpg';
  }
  function lekBildErsatz(l, klein, id){
    var e = ERSATZBILD[lekMotiv(l,id)]; if(!e) return '';
...
/* ---- Grundfläche ---- */
'.ku{--kb:#1F5FD1;--kb2:#12408F;--kr:#D42A21;--kg:#FFCE00;--kt:#12181F;--km:#5E6A78;--kl:#E7EBF0;max-width:1180px;margin:0 auto;padding-bottom:70px;color:#12181F}',
...
/* Die beiden Vollbilder hängen an <body>, nicht in .ku — ohne diese Zeile
   ...
   Eingabefeld ragt auf dem Handy rechts aus dem Bild. */
'.ku2-ov,.ku2-ov *,.sw-ov,.sw-ov *{box-sizing:border-box}',

/* ---- Knöpfe (auch in den Vollbildern gebraucht) ---- */
'.ku-btn{display:inline-flex;align-items:center;justify-content:center;gap:8px;border:none;cursor:pointer;font-family:inherit;font-weight:800;font-size:14.5px;padding:12px 20px;border-radius:13px;background:#D42A21;color:#fff;box-shadow:0 4px 0 #A80000;transition:transform .08s,box-shadow .08s;text-decoration:none}',
...
'.ku-lek.fertig .nr{background:#12883E;color:#fff}',
...
'.ku2-o.schlecht{border-color:#D42A21;background:#FFF4F4;color:#7A1414;font-weight:800}',
...
'.ku2-p.daneben{border-color:#D42A21;background:#FFF4F4}',
...
'.ku2-mic.laeuft{background:#D42A21;box-shadow:0 4px 0 #A80000;animation:kupuls 1.1s infinite}',
...
'.sw-falle{background:#FFF4F4;border:1px solid #F6D5D5;border-left:3px solid #D42A21;border-radius:0 12px 12px 0;padding:9px 12px;margin:0 0 7px;font-size:13.5px;color:#7A1414;line-height:1.45}',
...
'.ku-sw .angefangen{font-size:11.5px;font-weight:800;color:#0F7738}'
  ...
  /* ============================================================
     2 — Der Kurs eines Niveaus (A1, A2 …)
     ============================================================ */

  window.renderKurs=function(id){
    stil();
    ...
    var A=kursDaten(id)||{}, NV=niveauVon(id)||{}, p=niveauProzent(id);
    var zielKurz=String(A.ziel||NV.ziel||'').split('·')[0].replace(/^\s+|\s+$/g,'')||'—';
    var fertig=0,i; for(i=0;i<L.length;i++) if(lekProzent(L[i].nr,id)>=100) fertig++;

    var kopf='<div class="ku-kopf"><h1>'+E(A.titel||(id+' — Dein Kurs'))+'</h1>'
      +'<p>'+E(A.unter||'')+'</p>'
      +'<div class="ku-zahlen">'
      +'<div class="ku-z"><b>'+fertig+'/'+L.length+'</b><span>Lektionen fertig</span></div>'
      +'<div class="ku-z"><b>'+p+' %</b><span>vom ganzen Kurs</span></div>'
      +'<div class="ku-z"><b>'+E(zielKurz)+'</b><span>dein Ziel</span></div>'
      +'</div></div>';

    var weg='<div class="ku-weg">'+L.map(function(l){
      var lp=lekProzent(l.nr,id), teile=[];
      ...
      return '<button class="ku-lek'+(lp>=100?' fertig':'')+'" onclick="kursLektion('+l.nr+')">'
        +'<span class="bild">'+bildTag(lekBild(l,true,id),lekBildErsatz(l,true,id))
    ...
    v.innerHTML='<button class="ku-zurueck" onclick="renderNiveau()">← Alle Niveaus</button>'
      +kopf
      +'<h3 class="ku-h3">Dein Weg durch '+E(id)+'</h3>'
      +weg
  ...
  /* Der alte Name bleibt gueltig — start.js, lernen.js und lernbereich.js
     rufen ihn auf und geben dabei schon lange ein Niveau mit. */
  window.renderKursA1=function(id){ return window.renderKurs(id||null); };

  /* --- die Lektionsseite --- */
  var K={ lek:null, ki:0, gedreht:false };

  window.kursLektion=function(nr,id){
    stil();
    nivSetzen(id);
    var l=lektionVon(nr), v=flaeche(); if(!v) return;
    if(!l){ note('Diese Lektion finde ich gerade nicht.'); return window.renderKurs(); }
    ...
    var kopf='<div class="ku-hero">'
      +'<div class="ku-hero-tx">'
        +'<span class="nr">'+E(KURSNIV)+' · Lektion '+zahl(l.nr)+' von '+lektionen().length+'</span>'
        +'<h1>'+E(l.t)+'</h1>'
        +'<p>'+E(l.ziel||'')+'</p>'
    ...
    v.innerHTML='<button class="ku-zurueck" onclick="renderKurs()">← Alle Lektionen</button>'
      ...
      +(vor?'<div style="text-align:center;margin:22px 0 0">'
        +'<button class="ku-btn blau" onclick="kursLektion('+vor.nr+')">Lektion '+zahl(vor.nr)+': '+E(vor.t)+' →</button></div>':
  ...
  window.kuUebenStarten=function(){
    ...
    window.kursUeben(l.ueb, 'Lektion '+zahl(nr)+' · '+l.t, function(erg){
      ...
      if(el('v-kurs')||el('v-lernen')) window.kursLektion(nr);
    });
  };
  ...
  window.kuDialogStarten=function(){
    ...
    var d={
      id:zweig()+'-'+l.id,
      titel:E(KURSNIV)+' · Lektion '+zahl(l.nr)+' — '+l.t,
      lvl:KURSNIV,
  ...
  window.kuSchreibenStarten=function(){
    ...
    window.schreibTrainer({
      ...
      art:'mitteilung',
      pruef:'frei',
      t:E(KURSNIV)+' · Lektion '+zahl(l.nr)+' — Schreiben',
      sit:l.schreiben.auf||'',
      empf:'',
  ...
  window.swZu=function(){
    ...
    document.body.style.overflow='';
    W=null;
    if(zurueck){ lekMerken(zurueck,{schreiben:true}); if(el('v-kurs')||el('v-lernen')) window.kursLektion(zurueck); }
  };
  ...
  /* Der alte Name der Lektionsseite bleibt gueltig */
  window.kursA1=function(nr,id){ return window.kursLektion(nr,id); };
